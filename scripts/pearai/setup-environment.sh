...
app_dir=$(pwd)
target_path="$app_dir/extensions/treeai-submodule/extensions/vscode"
link_path="$app_dir/extensions/treeai-ref"

# Run the base functionality
echo -e "\nInitializing sub-modules..."

# Check if the submodule directory already exists
if [ -d "$app_dir/extensions/treeai-submodule" ]; then
    echo "Removing existing treeai-submodule directory"
    execute "rm -rf $app_dir/extensions/treeai-submodule" "Failed to remove existing treeai-submodule directory"
fi
...
execute "cd ./extensions/treeai-submodule" "Failed to change directory to extensions/treeai-submodule"
